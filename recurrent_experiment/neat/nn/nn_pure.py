...
def sigmoid(x, response, activation_type):
    ...
    try:
        if activation_type == 'exp':
            ...
            #raise NameError('Invalid activation type selected:', \
            #                activation_type)
...
class Neuron(object):
    ...
    __id = 0
    def __init__(self, neurontype, id = None, bias = 0.0, response = 1.0, \
                 activation_type = 'exp'):
        ...
        # for recurrent networks all neurons must have an "initial state"
        self._output = 0.0  

    type = property(lambda self: self._type, \
                    "Returns neuron's type: INPUT, OUTPUT, or HIDDEN")
    ...
    def activate(self):
        ...
        return sigmoid(self._update_activation() + self._bias, \
...
class Synapse(object):
    ...
    def __init__(self, source, destination, weight):
        self._weight = weight
        # a 'pointer' to the source neuron
        self._source = source
        # a 'pointer' to the destination neuron
        self._destination = destination
        # adds the synapse to the destination neuron
        destination.create_synapse(self) 
    ...
    def __repr__(self):
        return '%s -> %s -> %s' %(self._source._id, self._weight, \
...
class Network(object):
    ...
    def __repr__(self):
        return '%d nodes and %d synapses' % \
               (len(self.__neurons), len(self.__synapses))
...
def create_ffphenotype(chromo):
    ...
    neurons_list = [Neuron('INPUT', ng.id, 0, 0) \
                    for ng in chromo.node_genes[:chromo.sensors] \
                    if ng.type == 'INPUT']
